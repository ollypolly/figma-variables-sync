...
**Open, deliberately deferred: how much of this should be invisible vs. an explicit "local vs. synced" model.** Everything above leans toward automatic — minimize what a designer needs to understand, since the audience here is explicitly non-engineers. The tradeoff: a classic git-style local/origin mental model (pull/push, explicit sync state) is more predictable and inspectable when something doesn't do what someone expects, at the cost of asking designers to hold a concept they didn't sign up to learn. Not resolving this now — worth a dedicated think on its own before Slice 3 goes further, specifically: does the automatic-by-default design need an explicit "what's local vs. synced right now" escape hatch for when a designer gets confused, rather than deciding that implicitly by whatever gets built first. Also unresolved and connected: how a declined prompt should behave on repeat — re-ask every poll tick for the same unchanged staleness, or only when the situation actually changes (a new commit lands, the gap grows)? Leaning toward the latter (avoid nagging) but not settled.
