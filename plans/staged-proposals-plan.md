...
- **"Revert all" is the direct replacement for the Updates tab's "Accept Updates"** (see Slice 3) — same operation (git→Figma, wholesale), but branch-aware where Updates never was: it must diff/write against whatever's currently selected (main or the active PR's branch, via `resolveDiffSettings` — the same helper Proposals already uses) instead of a hardcoded `settings.branch`. Worth pulling forward ahead of per-row/per-group revert and staging if the Updates tab is being removed before the rest of this slice is ready — it doesn't depend on staging existing to be useful on its own.
