...
**3c. Is `main` ahead of what the designer last saw, while they're just browsing Main (no active PR)?** Same staleness signal as 3b, but there's no second branch to merge — you're just looking at main directly, so there's nothing for `update-branch` to do here, and nothing to decide either. Main is the shared canonical reference, not a branch with its own diverging history, so it should just always be live: silently auto-apply the safe (non-drifting) subset into Figma via the same mechanism `resetFigmaToGit` already provides (see Slice 1's "Reset"), scoped to fewer paths once that scoping exists. No prompt — a designer's own local experiments are never touched (see the mechanics note below), so there's nothing to ask permission for.

**3d. Designer changes which branch the plugin points at, in Settings.** Same silent auto-apply as 3c, different trigger (a deliberate Settings save instead of main quietly advancing) — no new mechanism of its own. Note this is specifically about the rarely-used Settings.branch field, not the PR dropdown — switching the dropdown to a *different PR* needs no special handling at all, it's just changing what's being diffed against; if that PR happens to be behind main, 3b's own prompt already covers it regardless of whether the designer just switched to it or had been sitting on it a while.
...
- Deletions are a real exception to "fully silent," unlike the false-alarm cases above — this one isn't about whose intent wins, it's about Figma-specific blast radius. Auto-applying a git-side delete means removing a Figma *variable*, not just changing a value, and if some other node/component is currently bound to it, that silently breaks a fill/binding elsewhere in the file with no warning. Worth excluding deletions from the silent path specifically — still auto-apply safe value changes/adds, but leave a safe-to-delete path as an ordinary visible pending item rather than silently removing it.
