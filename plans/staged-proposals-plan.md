...
**3c/3d, generalized: any diff-base change auto-applies the safe subset.** Originally scoped as "main is always live" (browsing main, or a Settings.branch change) with plain PR-switching treated as exempt — but that exemption doesn't hold up: switching to a *different PR* has the exact same safety property as switching to main (the no-drift guarantee doesn't care what the target branch is), so there's no real reason to treat them differently. The general rule: whenever what you're diffing against changes — a different PR selected, back to Main, a Settings.branch change, or 3a's fallback — Figma-apply the safe (non-drifting) subset via the same mechanism `resetFigmaToGit` already provides (see Slice 1's "Reset"), scoped to fewer paths once that scoping exists.

**Prompt by default, with an opt-out setting.** Given this is new and touches every branch switch, start with a confirmation ("This updates N variables to match {branch} — apply?") rather than fully silent, even though the underlying safety guarantee (never touches drifted paths) would support silent. Add a Settings toggle ("Skip this confirmation") for once it's trusted — only then does it become the fully-silent behavior originally planned. This toggle governs *only* the Figma-side safe-apply described here; it must not also silence 3b's confirmation below, since that one triggers a real git merge with real conflict potential, not a provably-safe local sync — a different risk category that should always ask regardless of this setting.
