...
**Still open, as separate follow-up PRs (deliberately not in PR1):** **3b** (real git merge via `update-branch`, `mergeable_state` polling, eject-to-dev, "Abandon this PR") — the one genuinely novel, higher-risk piece, needs no infrastructure from PR1 to stand alone later. Extending 3c/3d's switch-gate to the **Settings.branch edit** site — architecturally awkward (Settings tab holds no diff state today) and lower-frequency than the PR-tab sites.
...
- Deletions are a real exception, unlike the false-alarm cases above — this one isn't about whose intent wins, it's about Figma-specific blast radius, and it holds regardless of the confirmation/Settings-toggle question. Auto-applying a git-side delete means removing a Figma *variable*, not just changing a value, and if some other node/component is currently bound to it, that breaks a fill/binding elsewhere in the file. Worth excluding deletions from the safe-apply set entirely — even once "Skip this confirmation" is on, a safe-to-delete path should stay an ordinary visible pending item rather than being auto-applied at all.
